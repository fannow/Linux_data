...
Logger::Logger(string _name) :
	...
	//格式控制
	format.reset(new Formattor("%d%T%t%T%F%T[%p]%T[%c]%T%f:%l%T%m %n "));
}
...
class tabitemforma :public Formattor::Item {
public:
	typedef shared_ptr<Messageitemforma> ptr;
...
//	LOG_DEBUG(logger)<<"hello log";
	...
	//重新设置格式
	Formattor::ptr  fmt(new Formattor("%d%T%t%T%F%T[%p]%T[%c]%T%f:%l%T%m %n"));
	logger->setformat(fmt);
	logger->AddAppender(file);
	for (int i = 0;i < 1000000000;i++) {
		FMT_LOG_ERROR(logger, "nihap%s", "log");
	}
